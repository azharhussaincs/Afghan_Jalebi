...
const CAPITALS_GEO: Record<string, { capital: string; coord: [number, number]; isCapital?: boolean }> = {
  ...
  PKA: { capital: 'Sharana', coord: [68.7802, 33.1277] }
};
...
export const DynamicAfghanistanMap: React.FC<DynamicAfghanistanMapProps> = ({
  ...
  const chartOption = useMemo(() => {
    ...
    return {
      ...
      tooltip: {
        ...
        extraCssText: 'box-shadow: 0 10px 25px -5px rgba(0,0,0,0.8), 0 0 15px rgba(16,185,129,0.2); border-radius: 12px; backdrop-filter: blur(12px);',
          ...
          return `
            <div style="min-width: 220px; font-family: ui-sans-serif, system-ui, sans-serif;">
              <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 6px; margin-bottom: 8px;">
                <div>
                  <span style="font-size: 14px; font-weight: 800; color: #ffffff;">${gis.name}</span>
                  <span style="font-size: 13px; font-weight: 700; color: #34d399; margin-left: 6px;">${gis.nameDari}</span>
                </div>
                <span style="background: rgba(16,185,129,0.2); color: #34d399; font-size: 10px; font-weight: bold; padding: 2px 6px; border-radius: 4px; font-family: monospace;">${gis.id}</span>
              </div>
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px; font-size: 11px; margin-bottom: 8px;">
                <div><span style="color: #94a3b8;">Capital:</span> <span style="color: #e2e8f0; font-weight: 600;">${gis.capital}</span></div>
                <div><span style="color: #94a3b8;">Region:</span> <span style="color: #38bdf8; font-weight: 600;">${gis.region}</span></div>
                <div><span style="color: #94a3b8;">Area:</span> <span style="color: #cbd5e1;">${gis.area}</span></div>
                <div><span style="color: #94a3b8;">Elevation:</span> <span style="color: #fbbf24;">${gis.elevation}</span></div>
              </div>
              <div style="background: rgba(255,255,255,0.04); padding: 6px 8px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.06); margin-bottom: 6px;">
                <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 2px;">
                  <span style="color: #94a3b8;">Registered Records:</span>
                  <span style="color: #10b981; font-weight: 800; font-family: monospace;">${recFormatted}</span>
                </div>
                <div style="display: flex; justify-content: space-between; font-size: 10px; color: #64748b;">
                  <span>National Share:</span>
                  <span style="color: #38bdf8; font-family: monospace;">${pct}%</span>
                </div>
              </div>
              <div style="font-size: 10px; color: #34d399; text-align: center; font-style: italic;">
      ...
      visualMap: mapMode === 'choropleth' ? {
        ...
        bottom: 15,
        inRange: {
          color: ['#061a33', '#0369a1', '#0284c7', '#0d9488', '#10b981', '#34d399']
        },
        textStyle: {
          color: '#94a3b8',
          fontSize: 10,
  ...
  }, [mapMode, provinceMapData, selectedProvinceId, currentZoom, maxRecordCount, countLookup]);
  ...
  return (
    <div className={`relative w-full rounded-2xl overflow-hidden bg-[#070b16] border border-white/10 transition-all ${
      isFullscreen ? 'fixed inset-4 z-50 flex flex-col bg-[#070b16]/98 backdrop-blur-3xl shadow-2xl' : ''
    }`}>
      {/* Dynamic Map HUD Header Bar */}
      <div className="px-4 py-3 border-b border-white/10 bg-[#050811]/90 flex items-center justify-between flex-wrap gap-2.5">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-1.5 text-[11px] font-mono text-slate-400 border-l border-white/10 pl-3">
            <span className="text-emerald-400 font-bold">34 ADM1 Provinces</span>
            <span>&bull;</span>
            <span className="text-amber-400">Live Spatial Sync</span>
          </div>
        </div>

        {/* Layer Mode Switchers & Interaction Controls */}
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center bg-[#0d1424] p-1 rounded-xl border border-white/10 text-xs">
            <button
          ...
          {mapMode !== 'archival' && (
            <div className="flex items-center gap-1 bg-[#0d1424] p-1 rounded-xl border border-white/10">
              <button
          ...
          <button
            ...
            className="p-2 text-slate-400 hover:text-white bg-[#0d1424] hover:bg-white/10 rounded-xl border border-white/10 transition-colors"
      ...
      <div className={`relative w-full ${isFullscreen ? 'flex-1 min-h-0' : 'h-[520px]'}`}>
        {mapMode === 'archival' ? (
          <div className="w-full h-full flex flex-col items-center justify-center p-4 bg-black/80 overflow-auto">
            <img
              src="/afghanistan-map.png"
              alt="Afghanistan National Cartography Map"
              className="max-h-[460px] w-auto object-contain rounded-lg filter contrast-105 shadow-2xl border border-white/10"
            />
        ...
        {hoveredProvince && mapMode !== 'archival' && (
          <div className="absolute top-4 left-4 pointer-events-none bg-[#090e1a]/90 backdrop-blur-md border border-emerald-500/40 rounded-xl px-3.5 py-2 shadow-xl shadow-black/50 flex items-center gap-3 animate-fadeIn">
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping shrink-0" />
            <div>
              <div className="text-xs font-extrabold text-white flex items-center gap-2">
                <span>{hoveredProvince.name}</span>
                <span className="text-emerald-400 font-persian">{hoveredProvince.nameDari}</span>
                <span className="text-[10px] text-slate-400 font-mono">({hoveredProvince.id})</span>
              </div>
              <div className="text-[10px] text-slate-400 flex items-center gap-2 mt-0.5">
                <span>Capital: <strong className="text-slate-200">{hoveredProvince.capital}</strong></span>
                <span>&bull;</span>
                <span>Region: <strong className="text-cyan-400">{hoveredProvince.region}</strong></span>
                <span>&bull;</span>
                <span>Districts: <strong className="text-amber-400 font-mono">{hoveredProvince.districts}</strong></span>
              </div>
            </div>
          </div>
        )}
        ...
        {mapMode !== 'archival' && (
          <div className="absolute bottom-4 right-4 pointer-events-none hidden sm:flex items-center gap-2 bg-[#090e1a]/80 backdrop-blur-md border border-white/10 rounded-xl px-3 py-1.5 text-[11px] text-slate-400 shadow-lg">
        ...
        {mapMode === 'regional' && (
          <div className="absolute bottom-4 left-4 bg-[#090e1a]/90 backdrop-blur-md border border-white/10 rounded-xl p-2.5 shadow-xl text-xs space-y-1">
            <span className="font-bold text-[10px] uppercase text-slate-400 tracking-wider block mb-1">
              Geographic Regions
            </span>
