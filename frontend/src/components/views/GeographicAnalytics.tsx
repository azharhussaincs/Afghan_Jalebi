      ...
      series: [
        {
          ...
          data: processedDistricts.map((d) => ({
            ...d,
            itemStyle: { color: d.color, borderColor: '#020617', borderWidth: 2 }
          }))
        }
      ]
    };
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto text-slate-100">
      ...
      <div className="glass-card p-5 rounded-2xl border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-[#12192a] p-1 rounded-xl border border-white/10 shrink-0">
          <button
          ...
          >
            ...
            <span>34-Prov GIS Matrix &amp; Map</span>
          </button>
          <button
          ...
          >
            ...
            <span>Statistical Treemap &amp; Demographics</span>
          </button>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-5 rounded-2xl border border-white/10 space-y-4">
            ...
            <div className="flex items-center gap-1.5 flex-wrap pt-1 text-xs">
              <span className="font-bold text-slate-400 text-[11px] mr-1">Quick Jump:</span>
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all cursor-pointer ${
                    p.id === 'KBL'
                      ...
                      : 'bg-white/5 text-slate-300 hover:text-white border border-white/5 hover:border-emerald-500/30'
                  }`}
                >
          ...
          <div className="glass-card p-5 rounded-2xl border border-white/10 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-white">34-Province GIS Directory</span>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-56 bg-[#090e1a]/90 border border-white/10 focus:border-emerald-500 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white placeholder:text-slate-500 outline-none"
                  />
                </div>

                <div className="flex bg-[#090e1a] p-0.5 rounded-xl border border-white/10">
                  {regionsList.map(r => (
                    <button
                      key={r}
                ...
                return (
                  <div
                    ...
                    className={`glass-card p-4 rounded-xl border transition-all cursor-pointer hover:translate-y-[-2px] ${
                      isKabul
                        ? 'border-amber-500/50 bg-amber-500/[0.04] shadow-md shadow-amber-500/10'
                        : 'border-white/10 hover:border-emerald-500/40'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] font-bold px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-slate-300">
                        {p.id}
                      </span>
                    ...
                    <div className="mb-2">
                      <h4 className="text-sm font-bold text-white flex items-center justify-between">
                        <span>{p.name}</span>
                        <span className="font-persian text-xs text-emerald-400">{p.nameDari}</span>
                      </h4>
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        Capital: <strong className="text-slate-200">{p.capital}</strong>
                      </p>
                    </div>

                    <div className="grid grid-cols-3 gap-1 py-1.5 border-t border-b border-white/5 text-[10px] font-mono text-center">
                      <div>
                        <span className="text-slate-500 block text-[9px]">Districts</span>
                        <span className="text-slate-200 font-bold">{p.districts}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block text-[9px]">Area</span>
                        <span className="text-slate-200">{p.area.split(' ')[0]}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block text-[9px]">Elev</span>
                        <span className="text-cyan-400">{p.elevation}</span>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="glass-card p-5 rounded-2xl border border-white/10 flex flex-col justify-between">
              <div className="mb-2">
                <h3 className="text-sm font-bold text-white">Province Record Volume Ranking</h3>
            ...
            <div className="glass-card p-5 rounded-2xl border border-white/10 flex flex-col justify-between">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3 pb-3 border-b border-white/10">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold text-white">Top Cities &amp; Districts Treemap</h3>
                ...
                <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                  {/* Language Selector */}
                  <div className="flex items-center bg-[#090e1a] border border-white/10 rounded-lg p-0.5 text-xs">
                    <button
                      onClick={() => setDisplayMode('bilingual')}
                      className={`px-2 py-1 rounded text-[11px] font-medium transition-all ${
                        displayMode === 'bilingual' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
                      }`}
                    >
                      Bilingual
                    </button>
                    <button
                      ...
                      className={`px-2 py-1 rounded text-[11px] font-medium transition-all ${
                        displayMode === 'english' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
                      }`}
                    >
                      English
                    </button>
                    <button
                      ...
                      className={`px-2 py-1 rounded text-[11px] font-medium transition-all font-persian ${
                        displayMode === 'dari' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
                      }`}
                    >
                      دری
                    </button>
                  ...
                  {/* Count Selector */}
                  <div className="flex items-center bg-[#090e1a] border border-white/10 rounded-lg p-0.5 text-xs font-mono">
                    {[15, 25, 35].map((cnt) => (
                      <button
                        key={cnt}
                        onClick={() => setTopCount(cnt)}
                        className={`px-1.5 py-1 rounded text-[11px] font-medium transition-all ${
                          topCount === cnt ? 'bg-slate-800 text-emerald-300' : 'text-slate-500 hover:text-slate-300'
                        }`}
                      >
                        {cnt}
                      </button>
                  ...
                  <div className="flex items-center bg-[#090e1a] border border-white/10 rounded-lg p-0.5 text-xs">
                    <button
                      ...
                      className={`p-1.5 rounded transition-all ${
                        viewMode === 'treemap' ? 'bg-slate-800 text-emerald-400' : 'text-slate-500 hover:text-slate-300'
                      }`}
                    >
                      <LayoutGrid className="w-3.5 h-3.5" />
                    </button>
                ...
                <div className="h-80 overflow-y-auto pr-1 space-y-2">
                  {processedDistricts.map((d) => (
                    <div
                      ...
                      className="p-2.5 rounded-xl bg-[#090e1a]/80 hover:bg-[#12192a] border border-white/10 hover:border-emerald-500/50 transition-all cursor-pointer flex items-center justify-between gap-3"
                    >
                      ...
                      <div className="text-right shrink-0">
                        <div className="font-mono text-xs font-bold text-emerald-400">{d.exactCount}</div>
                        <div className="text-[10px] text-slate-400">{d.percentage}%</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-2 mt-2 border-t border-white/5 flex items-center justify-between text-[11px] text-slate-400">
          ...
          {data && (
            <div className="glass-card p-5 rounded-2xl border border-white/10">
              <h3 className="text-sm font-bold text-white mb-1">Province &times; Gender Distribution Matrix</h3>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-white/10 bg-[#090e1a]/90 text-slate-400 font-mono">
                      <th className="py-3 px-4">Province (ولایت)</th>
                      <th className="py-3 px-4">Code</th>
                      <th className="py-3 px-4 text-right text-cyan-400">Male (مرد)</th>
                      <th className="py-3 px-4 text-right text-pink-400">Female (زن)</th>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {data.provinces.map((p) => {
                      ...
                      return (
                        <tr key={p.province} className="hover:bg-white/[0.02] transition-colors">
                          <td className="py-2.5 px-4 font-bold text-white">
                            <div className="flex items-center gap-1.5">
                              <span>{enProv}</span>
                              <span className="text-slate-500 font-normal">&bull;</span>
                              <span className="font-persian text-emerald-400">{p.province}</span>
                            </div>
                          </td>
                          <td className="py-2.5 px-4 font-mono text-slate-400">{p.province_code}</td>
                          <td className="py-2.5 px-4 text-right font-mono text-cyan-400 font-medium">
                            {c0.toLocaleString()}
                          </td>
                          ...
                          <td className="py-2.5 px-4 text-center">
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-emerald-600 text-slate-300 hover:text-white text-[11px] transition-all"
                            >
                              Filter
                            </button>
                          </td>
      ...
      {selectedProvinceDossier && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="glass-card max-w-xl w-full rounded-2xl border border-white/15 p-6 space-y-4 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="bg-[#090e1a]/90 p-3 rounded-xl border border-white/10">
                <span className="text-slate-400 block text-[10px]">Native Script</span>
                <strong className="text-emerald-400 text-sm font-persian">{selectedProvinceDossier.nameDari}</strong>
              </div>
              <div className="bg-[#090e1a]/90 p-3 rounded-xl border border-white/10">
                <span className="text-slate-400 block text-[10px]">Capital Center</span>
                <strong className="text-white text-sm">{selectedProvinceDossier.capital}</strong>
              </div>
              <div className="bg-[#090e1a]/90 p-3 rounded-xl border border-white/10">
                <span className="text-slate-400 block text-[10px]">Geographic Region</span>
                <strong className="text-cyan-400">{selectedProvinceDossier.region} Afghanistan</strong>
              </div>
              <div className="bg-[#090e1a]/90 p-3 rounded-xl border border-white/10">
                <span className="text-slate-400 block text-[10px]">Districts Count</span>
                <strong className="text-white font-mono">{selectedProvinceDossier.districts} Districts</strong>
              </div>
              <div className="bg-[#090e1a]/90 p-3 rounded-xl border border-white/10">
                <span className="text-slate-400 block text-[10px]">Surface Land Area</span>
                <strong className="text-slate-200 font-mono">{selectedProvinceDossier.area}</strong>
              </div>
              <div className="bg-[#090e1a]/90 p-3 rounded-xl border border-white/10">
                <span className="text-slate-400 block text-[10px]">Median Elevation</span>
                <strong className="text-amber-400 font-mono">{selectedProvinceDossier.elevation}</strong>
              </div>
            </div>

            <div className="bg-[#090e1a]/90 p-4 rounded-xl border border-white/10 text-xs text-slate-300 leading-relaxed">
              <span className="text-slate-400 block text-[10px] font-bold uppercase mb-1">Regional Summary</span>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                onClick={() => setSelectedProvinceDossier(null)}
                className="px-4 py-2 bg-white/10 hover:bg-white/15 text-slate-300 hover:text-white font-semibold rounded-xl text-xs transition-all"
              >
                Close
              </button>
            </div>
