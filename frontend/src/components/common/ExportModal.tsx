...
const AVAILABLE_COLUMNS = [
  ...
  { id: 'hash_key', label: 'Hash Key (SHA-256)' }
];
...
export const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, defaultTab = 'records' }) => {
  ...
  const selectCoreColumns = () => setSelectedColumns(['id', 'name', 'fname', 'dob_year', 'gender', 'province', 'district']);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-brand-500/20 text-brand-300 text-[10px] font-mono">
                  v2.4 Engine
                </span>
              </h2>
        ...
        <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="flex items-center space-x-2 text-xs">
                <label className="flex items-center space-x-1.5 cursor-pointer text-slate-300 hover:text-white">
                  <input
                    ...
                    className="accent-brand-500 text-xs"
                  />
                  <span>Active Filters ({activeFilterCount})</span>
                </label>
                <label className="flex items-center space-x-1.5 cursor-pointer text-slate-400 hover:text-slate-200 ml-2">
                  <input
                    type="radio"
                    name="scopeType"
                    checked={!useActiveFilters}
            ...
            {useActiveFilters && activeFilterCount > 0 ? (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {filters.search_query && (
                  <span className="px-2 py-0.5 rounded bg-brand-950/80 border border-brand-500/40 text-brand-300 text-xs font-mono">
                    🔍 Search: "{filters.search_query}"
                  </span>
                )}
                {filters.province && (
                  <span className="px-2 py-0.5 rounded bg-brand-950/80 border border-brand-500/40 text-brand-300 text-xs">
                ...
                {filters.gender !== undefined && (
                  <span className="px-2 py-0.5 rounded bg-purple-950/80 border border-purple-500/40 text-purple-300 text-xs">
                    ⚧ {filters.gender === 0 ? 'Male (مرد)' : 'Female (زن)'}
                  </span>
                )}
                {(filters.dob_year_min || filters.dob_year_max) && (
                  <span className="px-2 py-0.5 rounded bg-cyan-950/80 border border-cyan-500/40 text-cyan-300 text-xs">
                    📅 Year: {filters.dob_year_min || 1250} - {filters.dob_year_max || 1405}
                  </span>
                )}
                {filters.book_name && (
                  <span className="px-2 py-0.5 rounded bg-amber-950/80 border border-amber-500/40 text-amber-300 text-xs truncate max-w-xs">
                    📖 {filters.book_name}
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-slate-400">
                {useActiveFilters
                  ? 'No specific filters active. Exporting cross-section of all 23,839,823 records.'
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center space-x-2 mb-1">
                  <Table className={`w-4 h-4 ${exportType === 'records' ? 'text-brand-400' : 'text-slate-400'}`} />
                  <span className="text-xs font-bold text-slate-100">Dataset Records (Rows)</span>
                </div>
              ...
              >
                <div className="flex items-center space-x-2 mb-1">
                  <FileText className={`w-4 h-4 ${exportType === 'summary' ? 'text-brand-400' : 'text-slate-400'}`} />
                  <span className="text-xs font-bold text-slate-100">Executive Briefing (PDF)</span>
                </div>
                <p className="text-[11px] text-slate-400">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                ...
                >
                  <div className="flex items-center space-x-1.5 mb-1">
                    ...
                    <span className="text-xs font-bold text-slate-100">Excel (.xlsx)</span>
                  </div>
                ...
                >
                  <div className="flex items-center space-x-1.5 mb-1">
                    ...
                    <span className="text-xs font-bold text-slate-100">PDF (.pdf)</span>
                  </div>
                  <p className="text-[10px] text-slate-400">
                ...
                >
                  <div className="flex items-center space-x-1.5 mb-1">
                    ...
                    <span className="text-xs font-bold text-slate-100">CSV (.csv)</span>
                  </div>
                  <p className="text-[10px] text-slate-400">
                ...
                >
                  <div className="flex items-center space-x-1.5 mb-1">
                    ...
                    <span className="text-xs font-bold text-slate-100">JSON (.json)</span>
                  </div>
                  <p className="text-[10px] text-slate-400">
          ...
          {exportType === 'records' && (
            <div className="border border-slate-800 rounded-xl overflow-hidden bg-slate-950/40">
              <button
                ...
                className="w-full p-3 flex items-center justify-between text-xs font-semibold text-slate-300 hover:text-white transition-colors"
              >
                ...
                <div className="p-4 border-t border-slate-800 bg-slate-950/80 space-y-3">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-2 py-1 rounded text-[11px] bg-slate-800 text-slate-200 hover:bg-slate-700"
                    >
                      Select All
                    </button>
                    <button
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            className="rounded border-slate-700 text-brand-500 focus:ring-brand-500/20 bg-slate-900"
                          />
          ...
          {/* Success Banner */}
          {downloadSuccess && (
            <div className="p-3 rounded-xl bg-emerald-950/40 border border-emerald-500/40 text-emerald-300 text-xs flex items-center space-x-2 animate-fade-in">
